...
public class DashboardController {
    ...
    public ResponseEntity<Map<String, Object>> stats() {
        ...
        long overdue = policies.stream()
                ...
                .count();
        long activePoints = pointOfSaleRepository.findAll().stream().filter(PointOfSale::isActive).count();
        long fraudAlerts = voucherRepository.findAll().stream().filter(Voucher::isFraudSuspected).count();
        Map<String, Object> result = new LinkedHashMap<>();
        result.put("totalPolicies", totalPolicies);
        result.put("averageDelinquency", totalPolicies == 0 ? 0.0 : Math.round((double) overdue / totalPolicies * 10000.0) / 100.0);
        result.put("activePointsOfSale", activePoints);
    ...
    public ResponseEntity<List<Map<String, Object>>> alerts() {
        ...
        List<Map<String, Object>> alertList = pointOfSaleRepository.findAll().stream()
                .map(pv -> {
                    long total = totalByPv.getOrDefault(pv.getId(), 0L);
                    long overdueCount = overdueByPv.getOrDefault(pv.getId(), 0L);
                    double ratio = total == 0 ? 0 : (double) overdueCount / total;
                    String level = ratio >= 0.20 ? "alto" : ratio >= 0.10 ? "medio" : null;
                    if (level == null) return null;
                    double[] coords = cityCoordinates.getOrDefault(pv.getCity(), new double[]{-12.0464, -77.0428});
                    Map<String, Object> alert = new LinkedHashMap<>();
                    alert.put("id", pv.getId());
                    alert.put("name", pv.getName());
                    alert.put("city", pv.getCity());
                    alert.put("sales", total);
                    alert.put("delinquency", Math.round(ratio * 1000.0) / 10.0);
                    alert.put("risk", level);
                    alert.put("lat", coords[0]);
                    alert.put("lng", coords[1]);
                    return alert;
                })
                .filter(Objects::nonNull)
                .sorted(Comparator.comparingDouble(m -> -((Number) m.get("delinquency")).doubleValue()))
                .collect(Collectors.toList());
